...
struct visualizer {
    ...
    void loadcases(std::string name) {
        ...
        while (std::getline(file, line)) {
            ...
            else if (action == "delay") cases.push_back(loadrunfromfunc("__delay", [&] (varray &arr, std::vector<std::variant<int, double>> args, std::string &name) {arr.high.delaymult = *std::get_if<double>(&args[0]);}, cache, {std::stod(nextarg(line, suiteargs))}, false));
